...
pkgdatadir = $(datadir)/yaim-storm
pkglibdir = $(libdir)/yaim-storm
pkgincludedir = $(includedir)/yaim-storm
top_builddir = .
...
DIST_COMMON = README $(am__configure_deps) $(srcdir)/Makefile.am \
	$(srcdir)/Makefile.in $(srcdir)/yaim-storm.spec.in \
...
CONFIG_HEADER = $(top_builddir)/etc/autogen/config.h
CONFIG_CLEAN_FILES = yaim-storm.spec
SOURCES =
DIST_SOURCES =
...
PACKAGE = yaim-storm
PACKAGE_BUGREPORT = 
PACKAGE_NAME = yaim storm
PACKAGE_STRING = yaim storm 4.1.7
PACKAGE_TARNAME = yaim-storm
...
etc/autogen/config.h: etc/autogen/stamp-h1
	@if test ! -f $@; then \
	  rm -f etc/autogen/stamp-h1; \
	  $(MAKE) etc/autogen/stamp-h1; \
	else :; fi

etc/autogen/stamp-h1: $(top_srcdir)/etc/autogen/config.h.in $(top_builddir)/config.status
	@rm -f etc/autogen/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status etc/autogen/config.h
$(top_srcdir)/etc/autogen/config.h.in:  $(am__configure_deps) 
	cd $(top_srcdir) && $(AUTOHEADER)
	rm -f etc/autogen/stamp-h1
	touch $@

distclean-hdr:
	-rm -f etc/autogen/config.h etc/autogen/stamp-h1
yaim-storm.spec: $(top_builddir)/config.status $(srcdir)/yaim-storm.spec.in
...
distdir: $(DISTFILES)
	$(am__remove_distdir)
	mkdir $(distdir)
	$(mkdir_p) $(distdir)/. $(distdir)/etc $(distdir)/etc/autogen $(distdir)/m4 $(distdir)/project
...
rpm: dist
	...
	/bin/cp -u yaim-storm-4.1.7.tar.gz $(RPM_SOURCE)
